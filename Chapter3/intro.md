...
### Plan-and-Solve工作原理
Plan-and-Solve核心动机是为了解决思维链在处理多步骤、复杂问题时容易偏离目标的问题。如ReAct这种边思考边行动的智能体架构在遇见复杂问题时，容易因为思考过程中出现的错误或偏差，导致最终结果偏离目标。而Plan-and-Solve则通过先规划再执行的范式，避免了这种问题的发生。
与ReAct不同的是，Plan-and-Solve先根据问题绘制出一个完整的解决方案（Plan），然后严格按照这个计划执行（Solve）。相当于将整个流程解耦为两个核心阶段，如image中的PAS.png所示：
...
---
具体而言，Plan-and-Solve范式的形式工作流程如下：

1. 首先规划模型$\pi_{plan}$根据原始问题q生成一个包含n个步骤的计划
$P = (p_{1}, p_{2}, ..., p_{n})$:
...
随后，在执行阶段，执行模型 $\pi_{solve}$ 会逐一完成计划中的步骤。对于第 $i$ 个步骤，其解决方案 $s_i$ 的生成会同时依赖于原始问题 $q$、完整计划 $P$ 以及之前所有步骤的执行结果 $(s_1, \dots, s_{i-1})$：
...
1. Planner: 负责根据用户问题生成一个解决问题的步骤列表。
2. Executor: 负责根据计划中的步骤，一步一步执行，调用工具或执行计算。


---
### Reflection
- 一种基于反馈的智能体架构，智能体在执行任务时，会根据任务完成度和结果，进行反思和调整，以提高任务执行效率和质量
- 相比较于ReAct与Plan-and-Solve范式，这两种智能体范式一旦完成了任务，其工作流便告终。然而，它们生成的初始答案，无论是行动轨迹还是最终结果，都可能存在谬误或有待改进之处
- Reflection机制的核心思想，正是为智能体引入一种事后的自我校正循环，使其能够像人类一样，审视自己的工作，发现不足，并进行迭代优化。

### Reflection机制的核心思想
- Reflection 机制的灵感来源于人类的学习过程：我们完成初稿后会进行校对，解出数学题后会进行验算。
- 其核心工作流可以概括为：执行 -> 反思 -> 优化。
...
### Reflection任务
- 编写一个Python函数，找出1到n之间所有的素数 (prime numbers)。这是一个绝佳场景，因为它需要同时考虑多个方面，包括逻辑推理、效率优化和边界条件处理。
Reflection 的核心在于迭代，而迭代的前提是能够记住之前的尝试和获得的反馈。因此，一个“短期记忆”模块是实现该范式的必需品。
